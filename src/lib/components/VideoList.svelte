          ...
          <div class="card-body {$layoutMode === 'compact' ? 'p-3' : ''}">
            <div class="flex items-center gap-2 mb-2">
              ...
              {#if v.is_published}
                ...
                <!-- Like count for published videos -->
                <div class="flex items-center gap-1 ml-auto">
                  <svg xmlns="http://www.w3.org/2000/svg" class="{$layoutMode === 'compact' ? 'h-3 w-3' : 'h-4 w-4'} text-error" fill="currentColor" viewBox="0 0 24 24" stroke="currentColor">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4.318 6.318a4.5 4.5 0 000 6.364L12 20.364l7.682-7.682a4.5 4.5 0 00-6.364-6.364L12 7.636l-1.318-1.318a4.5 4.5 0 00-6.364 0z" />
                  </svg>
                  <span class="{$layoutMode === 'compact' ? 'text-xs' : 'text-sm'}">{v.likes?.length || 0}</span>
                </div>
              {/if}
            </div>
