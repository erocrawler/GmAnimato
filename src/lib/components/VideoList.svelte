  ...
  <div class={gridClasses}>
    {#each Array(skeletonCount) as _}
      <div class="card bg-base-100 shadow-xl">
        <figure class="skeleton {layout === 'compact' ? 'h-32' : 'h-48'}"></figure>
        {#if layout === 'grid'}
...
{:else}
  ...
  <div class={gridClasses}>
    {#each videos as v}
      ...
      {:else}
        <!-- User/Admin card -->
        <div class="card bg-base-100 shadow-xl {layout === 'grid' ? 'image-full' : ''}">
          {#if v.original_image_url}
            <figure class={layout === 'compact' ? 'h-32' : ''}>
              <img src={v.original_image_url} alt="thumbnail" class="w-full h-full object-cover" />
            </figure>
          {:else}
            <figure class="bg-base-300 {layout === 'compact' ? 'h-32' : 'h-48'}">
          ...
          {/if}
          <div class="card-body {layout === 'compact' ? 'p-3' : ''}">
            <div class="flex items-center gap-2 mb-2">
              <div class="badge {layout === 'compact' ? 'badge-sm' : 'badge-lg'} {getStatusBadgeClass(v.status)}">
                {getStatusText(v.status)}
              </div>
              {#if type === 'admin'}
                {#if v.is_published}
                  <div class="badge badge-info {layout === 'compact' ? 'badge-sm' : ''}">Published</div>
                {/if}
              {/if}
            </div>
            ...
            {#if v.prompt && layout === 'grid'}
              <p class="text-sm line-clamp-2 mb-2">{v.prompt}</p>
            {/if}
            
            {#if v.status === "completed"}
              <div class="card-actions justify-end mt-auto gap-1">
                <a href="{type === 'admin' ? '/videos/' : '/videos/'}{v.id}" class="btn {layout === 'compact' ? 'btn-xs' : 'btn-sm'} btn-primary">{$_('videos.actions.view')}</a>
                {#if type === 'admin' && v.is_published && onUnpublish}
                  <button class="btn {layout === 'compact' ? 'btn-xs' : 'btn-sm'} btn-warning" onclick={() => onUnpublish?.(v.id)}>{$_('common.unpublish')}</button>
                {/if}
                {#if onDelete && v.status !== 'deleted'}
                  <button class="btn {layout === 'compact' ? 'btn-xs' : 'btn-sm'} btn-error" onclick={() => onDelete?.(v.id)}>{$_('common.delete')}</button>
                {/if}
              </div>
            {:else if v.status === "processing" || v.status === "in_queue"}
              <div class="card-actions justify-end mt-auto">
                <a href="/new/review/{v.id}" class="btn {layout === 'compact' ? 'btn-xs' : 'btn-sm'} btn-primary">{$_('review.viewStatus')}</a>
              </div>
            {:else if v.status === "uploaded"}
              <div class="card-actions justify-end mt-auto gap-1">
                <a href="/new/review/{v.id}" class="btn {layout === 'compact' ? 'btn-xs' : 'btn-sm'} btn-primary">{$_('review.continueSetup')}</a>
                {#if onDelete}
                  <button class="btn {layout === 'compact' ? 'btn-xs' : 'btn-sm'} btn-error" onclick={() => onDelete?.(v.id)}>{$_('common.delete')}</button>
                {/if}
              </div>
            {:else if v.status === 'failed'}
              <div class="card-actions justify-end mt-auto gap-1">
                <a href="/new/review/{v.id}" class="btn {layout === 'compact' ? 'btn-xs' : 'btn-sm'} btn-warning">{$_('videos.actions.retry')}</a>
                {#if onDelete}
                  <button class="btn {layout === 'compact' ? 'btn-xs' : 'btn-sm'} btn-error" onclick={() => onDelete?.(v.id)}>{$_('common.delete')}</button>
                {/if}
              </div>
