...
export async function buildFL2VWorkflow(params: FL2VWorkflowParams): Promise<object> {
  ...
  try {
    ...
    const resizeFirstInputs = getNodeInputs(workflow, resizeFirstNode);
    if (resizeFirstInputs) {
      resizeFirstInputs.width = gen480pWidth;
      resizeFirstInputs.height = gen480pHeight;
    }
    
    const resizeLastInputs = getNodeInputs(workflow, resizeLastNode);
    if (resizeLastInputs) {
      resizeLastInputs.width = gen480pWidth;
      resizeLastInputs.height = gen480pHeight;
    }
  ...
  // Find high/low UNet nodes for chain injection
  ...
  if (params.loraWeights && params.loraPresets) {
    ...
    if (!highUnetNode || !lowUnetNode) {
      ...
      throw new Error('Required FL2V LoRA chain UNet nodes not found in template');
    }
    ...
    if (highChain.length > 0) {
      ...
      if (!updated) {
        throw new Error('Unable to connect FL2V high LoRA chain to downstream model consumer');
      }
    }
